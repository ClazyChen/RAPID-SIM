...
public:
    ...
    void run_all(std::ostream& os, int packet_number) {
       // for (double lambda{ 0.1 }; lambda <= 0.9 + 1e-8; lambda += 0.1) {
        for (size_t arr_period{ 1 }; arr_period >= 1; arr_period--){
            std::array<std::stringstream, 10> m_results {};
            
            //#pragma omp parallel for
            for (int wb_gap { 0 }; wb_gap >= 0; wb_gap--) {
            //for (int i{ 1 }; i < 10; ++i) {
                ...
                Experiment<ImprovedSongPipeline<256, 32769, 4, 1024>, 32769> m_experiment;
                ...
                m_results[i] << " arr_period = " << arr_period << " ; wb_gap = " << wb_gap << std::endl;
